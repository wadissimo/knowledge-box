...
# Set up
folder = "sounds/cn"
voices = ["cmn-CN-Standard-A", "cmn-CN-Standard-B", "cmn-CN-Standard-C", "cmn-CN-Standard-D"]
data = read_csv('../import/eng_ch.csv', col_num=3)
log_file = "cn-log.txt"
WORD_IDX = 1
KEY_IDX = 0
PINYIN_IDX = 2
...
for row in data:
    word = row[WORD_IDX]
    key = row[KEY_IDX].lower()
    if key in key_set:
        continue
    key_set.add(key)
    words.append(word)
    pinyin.append(row[PINYIN_IDX])
...
if RUN_LOOP:
    ...
    with open(log_file, "a", encoding="utf-8") as log:
        for word, pinyin_word in zip(words,pinyin):
            try:
                ...
                # Insert Sound
                cur.execute("INSERT INTO sounds (ref, comment) values (?, ?);", (word, cur_voice + "_hanzi"))
                ...
                #Insert pinyin
                cur.execute("INSERT INTO sounds (ref, comment, file) values (?, ?, ?);", (pinyin_word, cur_voice + "_pinyin", file_name))
