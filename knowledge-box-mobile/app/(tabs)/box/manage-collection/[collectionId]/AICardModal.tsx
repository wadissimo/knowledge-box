...
import { View, Text, TextInput, ScrollView, Pressable, TouchableOpacity, ActivityIndicator, Modal } from "react-native";
  ...
  >
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "rgba(0,0,0,0.4)" }}>
      <View style={{ width: "90%", backgroundColor: "#fff", borderRadius: 18, padding: 20 }}>
        <Text style={{ fontSize: 22, fontWeight: "bold", marginBottom: 8 }}>{i18n.t("cards.askAiTitle") || "Ask AI to Generate Cards"}</Text>
        <Text style={{ fontSize: 15, marginBottom: 8 }}>{i18n.t("cards.askAiPrompt") || "Describe what cards you need:"}</Text>
        <TextInput
          style={{ borderWidth: 1, borderColor: Colors.light.border, borderRadius: 8, padding: 8, minHeight: 60, marginBottom: 10 }}
        ...
        {!aiGenerating && aiGeneratedCards.length > 0 && (
          <View style={{ marginBottom: 10 }}>
            <Text style={{ fontSize: 15, marginBottom: 8 }}>{i18n.t("cards.aiPreview") || "Review suggested cards:"}</Text>
            ...
            <View style={{ maxHeight: 300, position: 'relative' }}>
              ...
              >
                {aiGeneratedCards.map((card) => (
                  <View key={card.id} style={{ flexDirection: "row", alignItems: "flex-start", marginBottom: 8 }}>
                    <Pressable onPress={() => handleToggleAICard(card.id)} style={{ marginRight: 8, marginTop: 2 }}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}>
              ...
              >
                <Text style={{ color: "#fff", fontWeight: "bold" }}>{i18n.t("cards.save") || "Save"}</Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        {!aiGenerating && aiAmbiguous && (
          <View style={{ marginBottom: 10 }}>
            <Text style={{ fontSize: 15, marginBottom: 8 }}>{i18n.t("cards.noSuggestedCardsTitle") || "No Cards Suggested"}</Text>
            <Text style={{ color: 'red', marginVertical: 10 }}>{ambiguityMessages[aiAmbiguous] || ambiguityMessages.default}</Text>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}>
              <TouchableOpacity
                style={{ flex: 1, backgroundColor: Colors.light.tint, borderRadius: 8, padding: 10, marginRight: 8, alignItems: "center" }}
                onPress={handleAIGenerate}
              >
                <Text style={{ color: "#fff", fontWeight: "bold" }}>{i18n.t("cards.regenerate") || "Regenerate"}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{ flex: 1, backgroundColor: Colors.light.deleteBtn, borderRadius: 8, padding: 10, alignItems: "center" }}
        ...
        {!aiGenerating && aiErrorModal && (
          <View style={{ marginBottom: 10 }}>
            <Text style={{ fontSize: 15, marginBottom: 8 }}>{i18n.t("cards.aiError") || "AI Error"}</Text>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}>
              ...
              >
                <Text style={{ color: "#fff", fontWeight: "bold" }}>{i18n.t("cards.tryAgain") || "Edit Prompt & Try Again"}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{ flex: 1, backgroundColor: Colors.light.deleteBtn, borderRadius: 8, padding: 10, alignItems: "center" }}
        ...
        {!aiGenerating && aiGeneratedCards.length === 0 && !aiAmbiguous && (
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}>
            ...
            >
              <Text style={{ color: "#fff", fontWeight: "bold" }}>{i18n.t("cards.askAiBtn") || "Ask AI"}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={{ flex: 1, backgroundColor: Colors.light.deleteBtn, borderRadius: 8, padding: 10, alignItems: "center" }}
